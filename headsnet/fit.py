...
tss = 632847695 # fall back on hard coded size since I don't intend on changing it any time soon
if isfile('train_y.dat'): tss = int(os.stat("train_y.dat").st_size / 24)
print("Dataset Elements:", "{:,}".format(tss))
if dataset_limit > 0: print("Dataset Limit/Load Size:", "{:,}".format(dataset_limit))
...
for i in range(predict_points):
    fid.write(bytearray(struct.pack("fffccc",p[i][0],p[i][1],p[i][2],
        ...
        int(round(p[i][5]*255)).to_bytes(1, 'little'),
        )))
fid.close()
